...
| Test Scenario ID | Test Scenario | Functional Requirements | Acceptance Criteria | API liên quan | Số Test Case |
| --- | --- | --- | --- | --- | ---: |
| BP-01 | Quản lý tài khoản Customer | FR01–FR05 | AC01–AC05 | 01–05 | 20 |
| BP-02 | Quản lý Driver | FR06–FR10 | AC06–AC10 | 02, 04–08 | 20 |
| BP-03 | Tạo yêu cầu đặt xe | FR11–FR15 | AC11–AC15 | 10, 17 | 20 |
| BP-04 | Lựa chọn loại xe | FR16–FR17 | AC16–AC17 | 09, 10 | 20 |
| BP-05 | Tìm và lựa chọn Driver | FR18–FR21 | AC18–AC21 | 11, 12 | 20 |
| BP-06 | Driver tiếp nhận và phản hồi yêu cầu chuyến | FR22–FR25 | AC22–AC25 | 13, 14 | 20 |
| BP-07 | Theo dõi và thực hiện Trip | FR26–FR32 | AC26–AC32 | 15–17 | 20 |
| BP-08 | Tính cước và thanh toán | FR33–FR38 | AC33–AC38 | 15, 18 | 20 |
| BP-09 | Lịch sử chuyến và đánh giá Driver | FR39–FR42 | AC39–AC42 | 15, 19–21 | 20 |
...
| TC-BP01-005 | BP-01 – Quản lý tài khoản Customer | Negative | Đăng ký bằng email đã tồn tại | Email đã được sử dụng | 1. Nhập dữ liệu<br>2. Đăng ký | existing@example.com | Không tạo tài khoản; API trả 409 do email đã tồn tại | High |
| TC-BP01-006 | BP-01 – Quản lý tài khoản Customer | Negative | Đăng ký bằng phone đã tồn tại | Phone đã được sử dụng | 1. Nhập dữ liệu<br>2. Đăng ký | 0901234567 | Không tạo tài khoản; API trả 409 do phone đã tồn tại | High |
| TC-BP01-007 | BP-01 – Quản lý tài khoản Customer | Negative | Đăng nhập sai password | Customer tồn tại | 1. Nhập email đúng<br>2. Nhập password sai<br>3. Login | vana@example.com / Wrong123 | Không đăng nhập; API trả 401; không cấp token | High |
| TC-BP01-008 | BP-01 – Quản lý tài khoản Customer | Negative | Cập nhật email trùng tài khoản khác | Customer đã đăng nhập | 1. Sửa email<br>2. Lưu | existing@example.com | Không cập nhật; API trả 409 do email đã tồn tại | High |
| TC-BP01-009 | BP-01 – Quản lý tài khoản Customer | Boundary | Kiểm tra giới hạn validation khi được chốt | Cần có rule min/max chính thức | 1. Nhập dữ liệu tại giá trị biên<br>2. POST /auth/register | Min/max = TBD | Chưa thực thi cho đến khi giới hạn được Stakeholder phê duyệt; sau đó phải xử lý đúng giá trị biên | Medium |
...
| TC-BP01-016 | BP-01 – Quản lý tài khoản Customer | Empty | Không gửi token khi xem profile | Chưa xác thực | 1. Gọi GET /users/me | Authorization=empty | API trả 401 UNAUTHORIZED | High |
...
| TC-BP01-018 | BP-01 – Quản lý tài khoản Customer | Invalid Format/Type | Phone sai kiểu dữ liệu | API đăng ký hoạt động | 1. Gửi request đăng ký | phone=[] | API trả 400 do dữ liệu không hợp lệ | High |
| TC-BP01-019 | BP-01 – Quản lý tài khoản Customer | Invalid Format/Type | Email cập nhật sai định dạng | Customer đã đăng nhập | 1. PUT profile với email sai | abc@ | Không cập nhật profile; API trả 400 do email không hợp lệ | High |
| TC-BP01-020 | BP-01 – Quản lý tài khoản Customer | Invalid Format/Type | Token sai định dạng khi logout | Customer có token không hợp lệ | 1. Gọi logout | Bearer abc | API trả 401 UNAUTHORIZED | High |
...
| TC-BP03-007 | BP-03 – Tạo yêu cầu đặt xe | Negative | VehicleType không tồn tại | Customer đã login | 1. Tạo Trip | vehicleTypeId=VT999 | Không tạo Trip; API trả lỗi phù hợp theo contract | High |
...
| TC-BP03-012 | BP-03 – Tạo yêu cầu đặt xe | Boundary | Kiểm tra trạng thái biên của chính sách hủy khi được chốt | Cần có cancellation policy chính thức | 1. PUT /trips/{tripId}/cancel tại trạng thái biên | Trạng thái biên=TBD | Chưa thực thi cho đến khi cancellation policy được phê duyệt; sau đó xử lý đúng policy | Medium |
| TC-BP03-013 | BP-03 – Tạo yêu cầu đặt xe | Empty | Pickup address rỗng | Customer đã login | 1. POST Trip | pickupAddress=empty | Không tạo Trip; API trả lỗi phù hợp theo contract | High |
| TC-BP03-014 | BP-03 – Tạo yêu cầu đặt xe | Empty | Destination address rỗng | Customer đã login | 1. POST Trip | destinationAddress=empty | Không tạo Trip; API trả lỗi phù hợp theo contract | High |
| TC-BP03-015 | BP-03 – Tạo yêu cầu đặt xe | Empty | vehicleTypeId rỗng | Customer đã login | 1. POST Trip | vehicleTypeId=empty | Không tạo Trip; API trả lỗi phù hợp theo contract | High |
| TC-BP03-016 | BP-03 – Tạo yêu cầu đặt xe | Empty | Body tạo Trip rỗng | Customer đã login | 1. POST Trip | {} | API trả 400 do request không hợp lệ | High |
...
| TC-BP03-019 | BP-03 – Tạo yêu cầu đặt xe | Invalid Format/Type | tripId cancel không tồn tại | Customer đã login | 1. Cancel Trip | TRIP999 | API trả 404 do tài nguyên không tồn tại | High |
...
| TC-BP04-007 | BP-04 – Lựa chọn loại xe | Negative | Sử dụng Vehicle Type không hợp lệ để tạo Trip | Customer đã login | 1. POST Trip | vehicleTypeId invalid | Không tạo Trip; API trả lỗi phù hợp theo contract | High |
| TC-BP04-008 | BP-04 – Lựa chọn loại xe | Negative | Tạo Trip với Vehicle Type không tồn tại | Customer đã login | 1. POST /trips với vehicleTypeId không tồn tại | vehicleTypeId=VT999 | API trả 404 VEHICLE_TYPE_NOT_FOUND; không tạo Trip | High |
...
| TC-BP04-016 | BP-04 – Lựa chọn loại xe | Empty | Body Trip thiếu vehicleTypeId | Customer đã login | 1. POST Trip | không có vehicleTypeId | API trả 400 do request không hợp lệ | High |
...
| TC-BP05-014 | BP-05 – Tìm và lựa chọn Driver | Empty | driverId rỗng | Trip hợp lệ | 1. POST selection | driverId=empty | Không tạo TripOffer; API trả 400 do driverId không hợp lệ/rỗng | High |
...
| TC-BP06-015 | BP-06 – Driver tiếp nhận và phản hồi yêu cầu chuyến | Empty | Body response rỗng | Offer PENDING | 1. PUT {} | {} | API trả 400 do request không hợp lệ | High |
...
| TC-BP06-019 | BP-06 – Driver tiếp nhận và phản hồi yêu cầu chuyến | Invalid Format/Type | offerId không tồn tại | Driver login | 1. PUT response | OFF999 | API trả 404 do tài nguyên không tồn tại | High |
...
| TC-BP07-019 | BP-07 – Theo dõi và thực hiện Trip | Invalid Format/Type | tripId không tồn tại | User login | 1. GET/PUT Trip | TRIP999 | API trả 404 do tài nguyên không tồn tại | High |
...
| TC-BP08-006 | BP-08 – Tính cước và thanh toán | Positive | Thanh toán ELECTRONIC thành công | Trip COMPLETED, có finalFare và chức năng thanh toán điện tử MVP hoạt động | 1. POST /trips/{tripId}/payments | method=ELECTRONIC | Tạo Payment và lưu kết quả giao dịch | High |
...
| TC-BP08-008 | BP-08 – Tính cước và thanh toán | Negative | Thanh toán khi chưa đăng nhập | Không có token hợp lệ | 1. POST /trips/{tripId}/payments | Authorization=empty; method=CASH | API trả 401 UNAUTHORIZED | High |
| TC-BP08-009 | BP-08 – Tính cước và thanh toán | Negative | Trip COMPLETED nhưng finalFare chưa có | Dữ liệu không hợp lệ | 1. POST payment | CASH | Không tạo Payment; API trả 409 FINAL_FARE_NOT_AVAILABLE | High |
| TC-BP08-010 | BP-08 – Tính cước và thanh toán | Negative | Thanh toán Trip không thuộc Customer hiện tại | Trip thuộc Customer khác | 1. POST /trips/{tripId}/payments | method=CASH | API trả 403 FORBIDDEN; không tạo Payment | High |
...
| TC-BP08-013 | BP-08 – Tính cước và thanh toán | Boundary | Thanh toán ngay sau khi finalFare được ghi nhận | Trip vừa COMPLETED và đã có finalFare | 1. GET Trip xác nhận finalFare<br>2. POST payment | method=CASH | Payment được tạo với amount đúng bằng finalFare | Medium |
...
| TC-BP08-016 | BP-08 – Tính cước và thanh toán | Empty | Body payment rỗng | Trip COMPLETED | 1. POST payment | {} | API trả 400 do request không hợp lệ | High |
...
| TC-BP08-019 | BP-08 – Tính cước và thanh toán | Invalid Format/Type | method ngoài enum được hỗ trợ | Trip COMPLETED | 1. POST /trips/{tripId}/payments | method=CRYPTO | API trả 400 INVALID_PAYMENT_METHOD; không tạo Payment | High |
| TC-BP08-020 | BP-08 – Tính cước và thanh toán | Invalid Format/Type | tripId không tồn tại | Customer login | 1. POST payment | TRIP999 | API trả 404 do tài nguyên không tồn tại | High |
...
| TC-BP09-006 | BP-09 – Lịch sử chuyến và đánh giá Driver | Negative | Đánh giá trước khi Trip COMPLETED | Trip IN_PROGRESS | 1. POST rating | score hợp lệ | API trả 409 TRIP_NOT_COMPLETED | High |
...
| TC-BP09-008 | BP-09 – Lịch sử chuyến và đánh giá Driver | Negative | Đánh giá lần hai cùng Trip | Trip đã có Rating | 1. POST rating lần 2 | score hợp lệ | API trả 409 RATING_ALREADY_EXISTS | High |
...
| TC-BP09-012 | BP-09 – Lịch sử chuyến và đánh giá Driver | Boundary | Kiểm tra score tại min/max khi thang điểm được chốt | Cần có thang điểm Rating chính thức | 1. POST rating tại giá trị min/max | min/max=TBD | Chưa thực thi cho đến khi thang điểm được phê duyệt; sau đó xử lý đúng giá trị biên | Medium |
...
| TC-BP09-016 | BP-09 – Lịch sử chuyến và đánh giá Driver | Empty | Trip chưa có Rating khi GET | Trip thuộc Customer | 1. GET rating | TRIP001 | API trả 404 RATING_NOT_FOUND | Medium |
...
| TC-BP09-019 | BP-09 – Lịch sử chuyến và đánh giá Driver | Invalid Format/Type | tripId không tồn tại | Customer login | 1. GET/POST rating | TRIP999 | API trả 404 do tài nguyên không tồn tại | High |
